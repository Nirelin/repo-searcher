...
class FilterTab extends React.Component<IFilterTabProps> {
  ...
  public scrolledDownHandler = () : void => {
    ...
    !isAddRepsLoading && searchResultAmount && page<=searchResultAmount/24) {
       this.props.nextPage() 
    }
  }
  ...
  public numValid = (event: React.KeyboardEvent<HTMLInputElement>) : void => {
    if (event.key === '.'||event.key === '-'||event.key === ','||event.key === 'e'){event.preventDefault()}
  }

  public onDateChangeHandler = (value: Moment) : void => value===null ? this.props.changeFilter('updatedAfter', '') 
  :this.props.changeFilter('updatedAfter', value);
  
  public onTopicChangeHandler = (event: CheckboxChangeEvent) : void => this.props.changeFilter('hasTopics', event.target.checked);

  public onOrderChangeHandler = (value: string) : void => this.props.changeFilter('sortOrder', value);
      ...
      <div className='filter-bar'>
        <label>Sort by:
          <Select className="filter-antd-option" 
            ...
            disabled={!!this.props.searchResultAmount && this.props.searchResultAmount > 10000000} 
        ...
        <label>Updated after:
          <DatePicker 
            ...
            value={this.props.updatedAfter==='' ? undefined : this.props.updatedAfter}
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
    ...
    changeFilter: (filterName: string, filterValue: string|boolean) => dispatch(actions.changeFilter(filterName, filterValue)),
